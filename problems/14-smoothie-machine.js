...
const smoothieMachine = (...ingredients) => {
  let arr = [];
  return  function smoothie (...elements){
    for (i = 0; i < elements.length; i++){
    arr.push(elements[i]);
  }
    if (ingredients.length === 0){
          return `I'm having a smoothie with ${arr.join(' and ')}`
      } else {
          return `I'm having a smoothie with ${ingredients.join(' and ')} and ${arr.join(' and ')}`
      }
  }
};
